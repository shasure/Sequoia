...
def more_volume(code_name, data, end_date=None, threshold=60):
    """放量上涨；港股和A股不一样的是没有涨停；特殊情况，如果是庄股，放量比例可以放宽
    ...
    # hyper params
    MIN_UP_RATIO = 4  # 最小上涨
    MIN_TOTAL_TRADE_VALUE = 1.5 * 10 ** 8
    MIN_VOL_RATIO = 2
    MIN_VOL_RATIO_MM = 2.5  # 庄股放量
    ...
    if vol_ratio >= MIN_VOL_RATIO_MM and market_maker(data, threshold):  # 4倍
    ...
    if vol_ratio >= MIN_VOL_RATIO and not market_maker(data, threshold):  # 2倍
        ...
        logging.debug(vol_ratio)
        return True
    elif mean_vol_ratio >= MIN_VOL_RATIO and not market_maker(data, threshold):  # 2倍
